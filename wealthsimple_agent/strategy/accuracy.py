...
class AccuracyReport:
    ...
    avg_signal_return: float  # average forward return of all non-hold signals
    cost_adjusted_hit_rate: float  # assumes ~0.2% all-in cost per round-trip
    horizon_days: int
    threshold_pct: float
    disclaimer: str
...
def measure_accuracy(
    *,
    ...
    for i in range(min_lookback, len(bars) - horizon_days):
        ...
        signal_returns.append(fwd if sig.action == "buy" else -fwd)
        ...
        # After fees, did the signal direction still win?
        signed_return = fwd if sig.action == "buy" else -fwd
        if signed_return >= round_trip_cost_pct:
            cost_adjusted_correct += 1

    hit_rate = (correct / samples) if samples > 0 else 0.0
    avg_fwd = (sum(fwd_returns) / len(fwd_returns)) if fwd_returns else 0.0
    avg_signal_ret = (sum(signal_returns) / len(signal_returns)) if signal_returns else 0.0
    cost_adjusted_hit_rate = (cost_adjusted_correct / samples) if samples > 0 else 0.0
    ...
    return AccuracyReport(
        ...
        avg_signal_return=avg_signal_ret,
        cost_adjusted_hit_rate=cost_adjusted_hit_rate,
