...
def _user_id(store: Store) -> int:
    user = store.create_user(username=f"paper_test_{id(store)}", password="password123")
    return user["id"]
...
def test_paper_buy_sell_and_portfolio(tmp_path: Path):
    ...
    with Store(db) as store:
        ...
        reset = reset_paper_account(store, user_id=user_id, starting_cash=1_000.0)
        ...
        snap = portfolio_snapshot(store, user_id=user_id, price_overrides={"AAPL": 110.0})
...
def test_sell_without_position_fails(tmp_path: Path):
    ...
    with Store(db) as store:
        user_id = _user_id(store)
        reset_paper_account(store, user_id=user_id, starting_cash=500.0)
        try:
            execute_test_trade(
                store,
                user_id=user_id,
                req=PaperTradeRequest(ticker="MSFT", action="sell", quantity=1.0, price=300.0, db_path=str(db)),
            )
